...
%     [ trace, env, stem, indir ] = eval('analyse.load_xdata(xid,subject,game,kwargs)');
    ...
    for trial=1:1
        ...
        x0 =  (x0range(:,2)-x0range(:,1)).*ones(size(x0range,1),1)*0.5+x0range(:,1);
        problem = createOptimProblem('fmincon','x0',x0,'objective',fitnessfn,'lb',x0range(:,1),'ub',x0range(:,2),'options',options);
%         [x fval eflag output] = fmincon(problem);
%         gs = GlobalSearch('MaxTime',200,'Display','off');
        ms = MultiStart('MaxTime',200,'Display','off');
        [x,score] = run(ms,problem,10);
        [score fval]=fitnessfn(x);
...
function [ score logprob ] = agent_fitness(agenttype,numstates,numactions,trace,varargin)
    %varargin = varargin
    kwargs = utils.dict(varargin{:});
    ...
    if kwargs.iskey('epsilon')
        score=score - log(betapdf(2*kwargs.get('epsilon'),2,2));
    ...
    if kwargs.iskey('gamma')
        score=score - 2*log(betapdf(kwargs.get('gamma'),2,2));
    end
    if isnan(score)
        score=1e12;
    end
