...
x0=[5 0.41,0.2];
fitnessfn=@(x)hmm_logprob(x(1),x(2),0.01,x(3),actions(states==1),rewards_visit,indices_estimate,env,game);
lb=[1 0.4 0];
ub=[10 1 0.2];
...
[logprob gamma]=hmm_logprob(x(1),x(2),0.01,x(3),actions(states==1),rewards_visit,indices_estimate,env,game);
end
...
for i=2:length(indices_estimate)
    ...
    b(:,i)=[log((delta(1,1)+delta(2,1)));log((delta(1,2)+delta(2,2)))];
end
...
logprob=-sum(last)- 10*log(exppdf(sigma,1));
else
    logprob=+Inf;
end
catch err
