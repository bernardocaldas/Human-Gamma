    ...
    case 2
        ...
        switch game
            ...
            case 3
                ...
                env.get_gamma =@(r,p2,p3)real(max(0,min(-(5^(1/2)*(400*p2*(1-p3)-400*(1-p3)-120*p2+16*(1-p3)*r+5*p2^2-16*p2*(1-p3)*r+160)^(1/2)-5*p2+20)/(2*(p2-1)*((1-p3)*r-25*(1-p3)+5)),1)));
            ...
            case 4
                ...
                env.get_gamma =@(r,p2,p3)real(max(0,min(-((1700*p2*(1-p3)-1700*(1-p3)-714*p2+6800*(1-p3)*r+25*p2^2-6800*p2*(1-p3)*r+833)^(1/2)-5*p2+17)/(2*(p2-1)*(100*(1-p3)*r-25*(1-p3)+8)),1)));
                env.reward_state = 3;
                env.reward_action = 2;
            case 5
                % get gamma from immediate reward (reward indicates what gamma for policy switch)
                %env.get_gamma = @(r) min(0.25/(r-0.25),1);
                %XXX new analysis gives
                %             env.get_gamma = @(r,p) max(0,min(-395.0/(150.0*p - 1500.0*p*r + 169.0),1));
                env.get_gamma = @(r,p)real( max(0,min(-1.0/(p - 5.0*p*r),1)));
            ...
            case 6
                ...
                %XXX new analysis gives
                %             env.get_gamma = @(r) min((sqrt(17*(400*r - 51)) + 17)/(2*(100*r - 17)),1);
                env.get_gamma =@(r,p2,p3)real(max(0,min((0.5*((4.0*p2 + 4.0*p3 + 32.0*r - 4.0*p2*p3 - 32.0*p2*r - 32.0*p3*r + 32.0*p2*p3*r - 3.0)^(1/2) + 1.0))/(p2 + p3 + 8.0*r - 1.0*p2*p3 - 8.0*p2*r - 8.0*p3*r + 8.0*p2*p3*r - 1.0),1)));
                env.reward_state = 3;
                env.reward_action = 2;
            otherwise
                error('Unrecognised game type');
